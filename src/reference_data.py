...
class ReferenceDataManager:
    ...
    def download_reference(self, ref_type: str) -> Optional[Path]:
        ...
        if cache_file.exists():
            ...
            if age_days < 30:
                logger.info(f"Using cached SERPRO {ref_type} data ({age_days:.1f} days old)")
                return cache_file
        ...
        try:
            ...
            if cache_file.exists():
                logger.warning(f"Using stale cache due to download failure")
                return cache_file
            return None
    ...
    def diff_motivos_data(self, existing_codes: Set[str]) -> Optional[pl.DataFrame]:
        ...
        try:
            ...
            if "codigo" not in serpro_df.columns:
                logger.error(f"SERPRO file missing 'codigo' column. Found: {serpro_df.columns}")
                return None
            
            # Select and clean columns
            serpro_df = serpro_df.select([
                pl.col("codigo").cast(pl.Utf8).str.strip_chars(),
                pl.col("descricao").cast(pl.Utf8).str.strip_chars() if "descricao" in serpro_df.columns 
            ...
            serpro_df = serpro_df.with_columns([
                pl.col("descricao")
                    ...
                    .map_elements(remove_accents, return_dtype=pl.Utf8)  # Then remove accents
                    .alias("descricao")
            ])
            ...
            if len(missing_df) > 0:
                logger.info(f"Found {len(missing_df)} missing motivos codes in SERPRO data")
